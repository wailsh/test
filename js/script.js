...
/* ── Size Grouping (shared: Quick View + Product Page) ── */
var SIZE_GROUPS = [
    { label: 'Taille 1', prefix: 'taille 1', min: 36, max: 40 },
    { label: 'Taille 2', prefix: 'taille 2', min: 42, max: 46 },
    { label: 'Taille 3', prefix: 'taille 3', min: 48, max: 52 }
];

function getSizeGroup(sizeName) {
    var num = parseInt(sizeName, 10);
    if (isNaN(num)) return null;
    for (var i = 0; i < SIZE_GROUPS.length; i++) {
        if (num >= SIZE_GROUPS[i].min && num <= SIZE_GROUPS[i].max) return SIZE_GROUPS[i];
    }
    return null;
}
    ...
    return '<div class="product-card">' +
        '<div class="product-image">' +
            ...
            '<div class="product-overlay">' +
                ...
                '<button class="btn-overlay btn-overlay-primary" onclick="addToCart(' + pid + ')">Ajouter au panier</button>' +
            '</div>' +
        '</div>' +
...
function renderCartPage() {
    ...
    container.innerHTML = cart.map(function(item) {
        ...
        return '<div class="cart-item">' +
            '<div class="cart-item-image"><img src="' + (item.images && item.images[0] ? item.images[0] : item.image) + '" alt="' + esc(item.name) + '" loading="lazy"></div>' +
            '<div class="cart-item-details">' +
...
function productHasSizeInGroups(product, groups) {
    ...
    for (var i = 0; i < product.variants.length; i++) {
        ...
        for (var j = 0; j < sizesArr.length; j++) {
            ...
            var grp = getSizeGroup(szName);
            if (grp && groups.indexOf(grp.label) !== -1) return true;
            // Check by text prefix (e.g. "TAILLE 1", "taille 2")
            var lower = szName.toLowerCase();
            for (var k = 0; k < SIZE_GROUPS.length; k++) {
                if (groups.indexOf(SIZE_GROUPS[k].label) !== -1 && lower.indexOf(SIZE_GROUPS[k].prefix) !== -1) {
                    return true;
                }
            }
        }
